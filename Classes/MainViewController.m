...
- (IBAction) playOrPauseMusic: (id)sender {
	NSLog(@"playOrPauseMusic PB pressed");
	MPMusicPlaybackState playbackState = [musicPlayer playbackState];

	if (playbackState == MPMusicPlaybackStateStopped || playbackState == MPMusicPlaybackStatePaused) {
		musicPlayer.repeatMode = MPMusicRepeatModeAll;			// makes songs repeat in a loop 
...
- (void)createPopover
{
	NSLog(@"Music TableView popover should display");
	MusicTableViewController *musicTableView = [[MusicTableViewController alloc] init];
	musicTableView.delegate = self;
	
	// Creates the popoverController object
	musicTablePopover = [[UIPopoverController alloc] initWithContentViewController:musicTableView];
	[musicTablePopover setDelegate:self];
	//The parameters of CGRectMake are x, y, width and height
	[musicTablePopover presentPopoverFromRect:CGRectMake(162.5, 237.5, 1, 1) inView:self.view permittedArrowDirections:0 animated:YES];
	[musicTablePopover setPopoverContentSize:CGSizeMake(300, 400)];	
}
...
- (IBAction) AddMusicOrShowMusic: (id) sender {   
	...
	NSLog(@"edit playlist PB pressed");
	// if the user has already chosen some music, display that list
	if (userMediaItemCollection) {
		NSLog(@"already have music");
		MusicTableViewController *controller = [[MusicTableViewController alloc] initWithNibName: @"MusicTableView" bundle: nil];
		controller.delegate = self;

		// Creates the popoverController object
		musicTablePopover = [[UIPopoverController alloc] initWithContentViewController:controller];
		[musicTablePopover setDelegate:self];

		[musicTablePopover presentPopoverFromRect:CGRectMake(512, 384, 1, -1) inView:self.view 
						 permittedArrowDirections:0 animated:NO];
		[musicTablePopover setPopoverContentSize:CGSizeMake(325, 475)];			
		
		[controller release];

	// else, if no music is chosen yet, display the media item picker
	} else {
		NSLog(@"don't have music yet");
				
	//This is the original code that displays the media picker	
		MPMediaPickerController *picker =
			[[MPMediaPickerController alloc] initWithMediaTypes: MPMediaTypeMusic];
		
		// displays an empty version of the musicTableView if there is NO media (ie simulators)
		if (picker == NULL) { 
//			NSLog(@"Couldn't find any media"); 
			[self createPopover];
		} else {			
...
//		NSLog(@"musicTablePopover should display because there's no table of selected data yet");
		picker.delegate = self;
		
		// Creates the popoverController object
		musicTablePopover = [[UIPopoverController alloc] initWithContentViewController:picker];
		[musicTablePopover setDelegate:self];
		//I tweaked the numbers below to position the popup above the radio widget and size it- Adon
		[musicTablePopover presentPopoverFromRect:CGRectMake(512, 384, 10, -1) inView:self.view 
						 permittedArrowDirections:0 animated:NO];
		[musicTablePopover setPopoverContentSize:CGSizeMake(325, 475)];	
...
{
	NSLog(@"skipping into the forward!!");
	[musicPlayer skipToNextItem];
}

- (IBAction) skipBackward:(id) sender
{
	NSLog(@"skipping backwards!!");
...
- (void) mediaPicker: (MPMediaPickerController *) mediaPicker didPickMediaItems: (MPMediaItemCollection *) mediaItemCollection {
	...
	NSLog(@"didPickMediaItems triggered");
	[self.musicTablePopover dismissPopoverAnimated:YES];
//	[mediaPicker dismissPopoverAnimated:YES];			//Added CPL 2/27 
...
- (void) mediaPickerDidCancel: (MPMediaPickerController *) mediaPicker {
	...
	NSLog(@"mediaPickerDidCancel method triggered");
	[self.musicTablePopover dismissPopoverAnimated:YES];
//	[self dismissModalViewControllerAnimated: YES];
...
//********************
	...
	if (musicPlayer.playbackState == MPMusicPlaybackStateStopped) {
		...
		//		finished playing.
		[nowPlayingLabel setText:[NSString stringWithFormat: @"Press + to create a playlist"]];
...
- (void) musicTableViewControllerDidFinish: (MusicTableViewController *) controller {
	...
	NSLog(@"musicTableViewControllerDidFinish invoked");
	[self.musicTablePopover dismissPopoverAnimated:YES];
...
// play button animation
	...
	[self setPlayedMusicOnce: NO];
										// Prevents display of original AddMusic screen CPL 2/16/11
	...
	[nowPlayingLabel setText: NSLocalizedString (@"Press Edit Playlist to hear music", @"Brief instructions to user, shown at launch")];
